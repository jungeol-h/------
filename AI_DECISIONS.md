#### [2026-06-03] 로그인 성공값으로 즉시 라우팅하고 루트 재진입 자동 이동

- **Context & Ambiguity:** 사용자가 "첫 로그인 버튼은 안 되고 두 번째 눌러야 되는지"와 "자동로그인이 되나"를 함께 물었다. 첫 클릭 버그 수정만 할 수도 있고, 이미 저장된 사용자로 `/`에 들어왔을 때 대시보드로 자동 이동하게 할 수도 있었다.
- **Your Choice & Action:** `login()`이 boolean 대신 로그인된 `user` 객체를 반환하게 하고, `LoginPage`는 `localStorage`를 다시 읽지 않고 그 객체의 role로 즉시 이동하게 했다. 또한 `currentUser`가 이미 있으면 로그인 페이지 진입 즉시 role별 대시보드로 `replace` 이동한다.
- **Reasoning & Justification:** 첫 클릭 버그의 직접 원인은 React state/localStorage 반영 타이밍 레이스였다. 성공 응답에서 받은 `user`를 그대로 쓰면 레이스가 사라진다. 저장된 사용자가 있는데 `/` 로그인 화면에 머무르는 것은 자동 로그인 기대와 맞지 않아 루트 재진입도 같은 흐름으로 처리했다.
- **Potential Risk / Review Required:** 이 앱은 Supabase Auth 세션 검증 없이 `localStorage`의 `platform_user`를 신뢰한다. 관리자가 DB에서 비활성화한 사용자도 기존 저장값이 남아 있으면 로그아웃 전까지 클라이언트상 로그인된 것처럼 보일 수 있으므로, 운영 보안 요구가 올라가면 앱 시작 시 서버 재검증이 필요하다.

#### [2026-06-03] 확인평가 리셋 긴급 대응을 localStorage 초안 복구로 제한

- **Context & Ambiguity:** 학생들이 확인평가 중 몇 문제를 푼 뒤 갑자기 리프레시되거나 페이지에서 나간다고 보고했지만, 실제 원인이 나가기 버튼 오탭인지 PWA 배포 업데이트인지 React 크래시인지는 확정되지 않았다. TanStack Query 도입도 언급됐지만 긴급 픽스 범위와 근본 개편 범위가 섞여 있었다.
- **Your Choice & Action:** 이번 변경은 `QuizTab` 내부의 localStorage 초안 저장/복구, 나가기 확인, beforeunload 경고로 제한했다. 서버 초안 테이블, TanStack Query 도입, PWA 업데이트 정책 변경은 제외했다.
- **Reasoning & Justification:** 현재 구현은 답안이 React state에만 있어 나가기 오탭·새로고침·배포 업데이트 모두에서 즉시 유실된다. localStorage 초안은 스키마 변경 없이 오늘 배포 가능한 최소 보호막이고, TanStack Query는 데이터 fetch/cache 구조 개선에는 유용해도 작성 중 답안 유실의 직접 해결책은 아니다.
- **Potential Risk / Review Required:** 초안은 같은 브라우저/기기에서만 복구된다. 공용 기기 사용 시 다른 사용자가 같은 계정으로 들어오면 이전 초안이 보일 수 있으므로, 운영 방식상 기기 공유가 잦다면 서버 저장 또는 명시적 초안 만료 정책을 검토해야 한다.
