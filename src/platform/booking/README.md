...
### 강사지정예약 (2026-07-30 클라이언트 요청)

강사가 학생·날짜·시간을 지정해 예약을 고정한다 (`DesignatedReserveModal`,
강사 예약관리 '내 슬롯' 메뉴). 학생 대리 예약(관리자)은 그대로 유지 — 별도 기능.

- **구현 = 비공개 슬롯(정원 1, note '강사지정') + 확정 예약**을 회차별 생성.
  새 테이블·RPC 없음. 매주 반복은 '종료일자'까지 7일 간격 실체화(상한 26회).
...
- **부분 실패**: 강사 본인 슬롯과 겹치는 회차는 미리보기에서 건너뛰고, 학생 쪽
  ...
  (booking_update_slot p_delete, 예약 없는 슬롯만 삭제 가능이라 안전).
