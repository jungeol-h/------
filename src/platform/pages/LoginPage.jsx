...
const ACTIVE_ROLES = [
  { role: 'student', label: '학생', icon: '🎒', color: 'bg-blue-500 hover:bg-blue-600', desc: '학습 현황 · 마인드 · 진로' },
  { role: 'manager', label: '학습매니저', icon: '👩‍💼', color: 'bg-emerald-500 hover:bg-emerald-600', desc: '학생 관리 · 코칭 · 알림' },
  { role: 'admin', label: '관리자', icon: '🏫', color: 'bg-violet-500 hover:bg-violet-600', desc: '통계 · 사용자 관리 · 리포트' },
]

const UPCOMING_ROLES = [
  { role: 'parent', label: '학부모', icon: '👪', desc: '준비 중' },
  { role: 'teacher', label: '강사', icon: '👨‍🏫', desc: '준비 중' },
  { role: 'consultant', label: '컨설턴트', icon: '🧑‍💻', desc: '준비 중' },
]
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="space-y-3 mb-6">
          {ACTIVE_ROLES.map(({ role, label, icon, color, desc }) => (
            <button
              key={role}
        ...
        <div className="grid grid-cols-3 gap-2">
          {UPCOMING_ROLES.map(({ role, label, icon, desc }) => (
            <div
              key={role}
              className="flex flex-col items-center gap-1 px-3 py-3 rounded-xl bg-white border border-gray-200 opacity-50 cursor-not-allowed"
            >
