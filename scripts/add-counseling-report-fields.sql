-- ============================================================
-- 상담 기록 보고서 양식 4단계 필드 마이그레이션
-- 사용: Supabase Studio → SQL Editor → 붙여넣기 후 RUN
-- ------------------------------------------------------------
-- 용도 (2026-07 클라이언트 요청 "상담 유형 5분야 + 내용 4단계"):
--  counseling_records / program_counseling_records 에
--  topic(주제) · detail(세부내용) · follow_up(후속조치) · note(특이사항) 추가.
--
--  * content 컬럼은 유지 — 신규 기록도 4필드 합성 텍스트를 함께 저장한다
--    (NOT NULL 유지, PDF·레거시 소비처 호환). 구 기록은 4필드 NULL로 남는다.
--  * type 값 체계 교체(admin/consultant/subject_coaching/mentoring/extracurricular)는
--    앱 레벨 변경이라 DB 마이그레이션 불필요 — 구 값(study/career/habit/mind/etc)은
--    라벨만 유지해 표시된다.
-- ------------------------------------------------------------
...
alter table counseling_records add column if not exists detail text;
...
alter table program_counseling_records add column if not exists detail text;
